...
function generateBooks(data) {
  data.forEach(function(value, key){
    if (!value["url"].toLowerCase().includes('user:')) {
      ...
      tmp += '<div class="col px-0 book"><div class="card"><div class="card-body"><h5 class="card-title"><a href="' + value["url"] + '" class="link">' + value["title"] + '</h5></a><div class="card-text">' + value["txt"].replace(preremove, "").replace('<p><br />', '<p>') + '</div></div></div></div>';
    }
  })
  $('#books').append(tmp);
}
