...
function generateBooks(data) {
  data.forEach(function(value, key){
    if (!value["url"].toLowerCase().includes('user:')) {
      ...
      tmp += '<div class="col px-0 book"><div class="card"><a href="' + value["url"] + '" class="stretched-link"></a><div class="card-body"><h5 class="card-title">' + value["title"] + '</h5><div class="card-text">' + value["txt"].replace(preremove, "").replace('<p><br />', '<p>') + '</div></div></div></div>';
    }
  })
  $('#books').append(tmp);
}
